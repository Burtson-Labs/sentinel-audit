...
async function runScan(args: Args): Promise<number> {
  ...
  const lines = [
    ...
    '',
    `  confirmed   ${String(count('confirmed')).padStart(3)}   (an executed check agreed)`,
    ...
    `  verified share of live findings: ${Math.round(confidence.evidenceQuality.verifiedShare * 100)}%`,
