...
- **The ESM entry-script check is no longer a path-traversal finding.**
  `resolve(process.argv[1]) === fileURLToPath(import.meta.url)` compares two
  strings and opens nothing, but it was reported as a High, plausible
  `SEC-PATH-TRAVERSAL`. `process.argv` used as an actual path is still reported.
